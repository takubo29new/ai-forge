...
Phase 1〜3が別々の機能ではなく、データを掛け合わせて初めて作れる機能に着手する。**設計完了。実装は未着手。**

設計ドキュメントを作成した(詳細は[`docs/phase4-design.md`](./docs/phase4-design.md)を参照)。要点:

- **RAG検索対象の拡張**: `Prompt`本文・`Execution`結果もベクトル埋め込みの対象に追加し、`/chat`から横断的に検索できるようにする(現状は`Document`・`ReviewComment`のみ)。以降の項目の土台になる。`Execution`はレビュー由来(`ReviewComment`と重複)を除外する設計
...
- **レビュー指摘蓄積からのプロンプト改善提案**: 上記の検索基盤の上に構築。独自のパターン検出ロジックは作らず、Claude自身に指摘一覧を分析させる方針(永続化はせず、都度生成)
