...
2026-08-24時点でユーザーと検討した今後の方向性をまとめる。優先順は上から。

### Phase 4: 統合基盤の強化

Phase 1〜3が別々の機能ではなく、データを掛け合わせて初めて作れる機能に着手する。

- **RAG検索対象の拡張**: `Prompt`本文・`Execution`結果もベクトル埋め込みの対象に追加し、`/chat`から横断的に検索できるようにする(現状は`Document`・`ReviewComment`のみ)。以降の項目の土台になる
- **プロジェクト単位のドキュメント管理**: `Document`に`repositoryId`(任意)を追加し、Phase 2で接続済みのGitHubリポジトリごとにドキュメントを同期・`/chat`で対象を絞り込めるようにする(詳細は[`docs/phase3-design.md`](./docs/phase3-design.md)の「今後の拡張候補」参照)
- **レビュー指摘蓄積からのプロンプト改善提案**: 上記の検索基盤の上に構築。同じ種類の指摘が繰り返されているプロンプトを検出し、改善案を提示する
- **チャットからの直接アクション実行**: 検索基盤の上に構築。「このリポジトリの最近のPRをレビューして」のような指示から、実際に保存済みプロンプトでレビューを起動できるようにする

### Phase 5: 汎用AI評価ツールへの拡張

コードレビューに限らず、写真(料理・絵など)やテキストを対象にAIが評価するツールへ対象を広げる、大きめの方向転換。**設計完了。実装は未着手。**

設計ドキュメントを作成した(詳細は[`docs/phase5-design.md`](./docs/phase5-design.md)を参照)。要点:

- 既存の`Review`(コードレビュー専用)は変更せず、新しく`Evaluation`/`EvaluationFinding`という並行した概念を追加する方針(統合リスクを避ける)
- 対応する入力形式は画像(Claude Vision)とテキストの2種類。Claude APIは音声そのものの解析には対応していないため、楽曲評価は歌詞・楽譜のテキスト化や説明文の入力が前提になる
- 画像はDB/ストレージに永続化せず、リクエスト内でClaudeに渡すのみ(スコープを絞るための判断。将来Vercel Blob等の導入を再検討)
- いきなり汎用化せず、画像評価など1ケースをプロトタイプ的に実装してから逆算する
