...
コードレビューに限らず、写真(料理・絵など)やテキストを対象にAIが評価するツールへ対象を広げる、大きめの方向転換。

- まず設計ドキュメントを作成する(評価対象・入力形式の抽象化、画像評価の実現方式、構造化出力スキーマの一般化)
- Claude APIはVision(画像)には対応しているが、音声そのものの解析には対応していない。楽曲評価は歌詞・楽譜のテキスト化や説明文の入力が前提になる点に注意
