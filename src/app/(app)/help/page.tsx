...
{"{{topic}}について、初心者向けに3行で説明してください。"}
        ...
        <section id="chat" className="scroll-mt-6">
          ...
          <p className="mb-2 text-sm text-zinc-700 dark:text-zinc-300">
            接続済みリポジトリと保存済みプロンプトの両方がある場合、「
            <code className="rounded bg-zinc-100 px-1 py-0.5 font-mono text-xs dark:bg-zinc-800">owner/repoのPR #12を「プロンプト名」でレビューして</code>
            」のように、リポジトリ名・PR番号・プロンプト名を含めて実行を依頼する文章を送ると、質問への回答の代わりに実行内容の確認画面が表示されます(入力欄の上に表示される案内の「この例文を入力欄にセット」ボタンからも試せます)。内容を確認して「実行」を押すと、そのままAIレビューが実行され、結果画面へのリンクが表示されます(「キャンセル」を押すか内容が違っていれば取り消せます)。リポジトリ名など特定できない情報がある依頼は実行の提案自体が行われません。それ以外の操作(リポジトリの接続解除やプロンプトの削除など)はチャットからは実行できません。
          </p>
          <p className="text-sm text-zinc-700 dark:text-zinc-300">
