...
{"{{topic}}について、初心者向けに3行で説明してください。"}
        ...
        <section id="chat" className="scroll-mt-6">
          ...
          <p className="mb-2 text-sm text-zinc-700 dark:text-zinc-300">
            接続済みリポジトリと保存済みプロンプトの両方がある場合、「target/repoのPR #12をコードレビュー用プロンプトでレビューして」のように実行を依頼する文章を送ると、質問への回答の代わりに実行内容の確認画面が表示されます。内容を確認して「実行」を押すと、そのままAIレビューが実行され、結果画面へのリンクが表示されます(「キャンセル」を押すか内容が違っていれば取り消せます)。それ以外の操作(リポジトリの接続解除やプロンプトの削除など)はチャットからは実行できません。
          </p>
          <p className="text-sm text-zinc-700 dark:text-zinc-300">
