...
1. **ユーザーと要件を確認する**: 入力形式(テキストのみか、画像などマルチモーダルか)、構造化出力の形(EvaluationのようなFinding配列か、単一の判定か)、既存ドメインとの関係(独立した新モデルか、既存モデルの拡張か)。既存2ドメインは「Reviewはコード差分専用のフィールドを持ち既存機能と深く統合されているため無理に汎用化しない」という判断をしている(docs/phase5-design.md参照) — 新ドメインも同様に、既存モデルを汎用化するより新設する方が安全な場合が多い。
...
9. **ドキュメントを更新する**: 対応するdocs/phaseN-design.mdのステータス、`docs/db-design.md`への新モデルの追記、`ai-dev-tool-handoff.md`・`README.md`の進捗欄を更新する。
