...
### Phase 5: 汎用AI評価ツール(画像・テキスト評価、バックグラウンド処理)

コードレビューに限らず、画像・テキストなど他の入力形式を評価する汎用AI評価ツールへの拡張。

- `Evaluation`/`EvaluationFinding`をReviewとは独立したモデルとして追加(コード専用のReviewを無理に汎用化しない設計判断)
- `/evaluations`から画像+プロンプトを送信し、Claude Visionで評価・観点別コメント(トーン・スコア)を取得。画像はDB/ストレージに永続化せず、リクエスト内でClaudeに渡すのみ
...
- プロンプトテンプレート集(`/prompts/new`の「テンプレートから始める」。料理写真・イラスト・歌詞・文章の評価用の叩き台プロンプトを選ぶとタイトル・本文が自動入力される)
