...
### Phase 5: 汎用AI評価ツール(画像評価プロトタイプ・バックグラウンド処理)
...
- Claude Vision呼び出しはNext.jsの`after()`でレスポンス送出後にバックグラウンド実行(新規ワーカー・キューは追加せず)。`(app)`レイアウトに常駐するプロバイダがポーリングし、完了を画面をまたいでトースト通知

テキスト評価・画像の永続化・共有リンクは設計のみで未着手。設計・実装状況は [`docs/phase5-design.md`](./docs/phase5-design.md) を参照。
