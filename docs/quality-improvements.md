...
対応を見送った指摘: GitHubアクセストークンの平文保存(NextAuth Prisma Adapterの標準仕様。実ユーザーを迎える段階でトークン暗号化を再検討)、ルートレベルの統合テスト(「DBアクセスを伴う処理は手動確認する」という1番の方針を踏襲。CI導入によりこのトレードオフの前提は変わりつつあるため、次の見直し候補)。
