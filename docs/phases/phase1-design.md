...
更新日: 2026-08-23 / ステータス: Phase 1完了。mainにマージ済み
...
詳細な画面遷移・ワイヤーフレームは [`docs/phase1-ui-design.md`](./phase1-ui-design.md) を参照。実装済みの画面は以下。
...
Phase 1のスコープ(プロンプトCRUD・AI実行・履歴/バージョン管理)は実装完了し、`main`にマージ済み。

| 項目 | 内容 |
| --- | --- |
| 画面遷移・UI設計 | [`docs/phase1-ui-design.md`](./phase1-ui-design.md) にワイヤーフレーム・画面遷移図をまとめた |
| ローカルDB環境 | `docker-compose.yml`でpgvector/pgvector:pg16イメージのPostgresを起動し、`prisma migrate dev`で初期スキーマを適用。pgvector拡張の有効化(`CREATE EXTENSION`)自体はPhase 3で行う |
| 認証 | `src/auth.ts`でNextAuth.js(v5) + PrismaAdapter + GitHub Providerを設定(セッションはdatabase方式)。Prisma 7ではドライバーアダプタが必須のため`@prisma/adapter-pg`を使用 |
| カテゴリCRUD | `GET/POST /api/categories`・`PATCH/DELETE /api/categories/:id`、`/categories`画面(追加・編集・削除。削除時は該当プロンプト件数を確認ダイアログで表示) |
