...
- **本番環境での注意**: DBがVercelのFunctionから地理的に離れていると、`POST /api/documents/sync`のトランザクション(ファイル数だけ発生するdeleteMany+create)がPrismaの既定のインタラクティブトランザクションタイムアウト(5秒)を超えることがある。`$transaction`に`{ timeout: 30000 }`を指定し、あわせてVercel Function自体の実行時間上限も`export const maxDuration = 60`で引き上げて対応している
...
- ~~複数リポジトリの同期・プロジェクト単位のRAG検索~~ → Phase 4項目2として実装完了。`Document`に`repositoryId`(任意)を追加し、Phase 2で接続済みの`Repository`ごとにGitHub API経由でREADME・docs/配下を同期、`/chat`でも対象リポジトリを絞り込めるようにした。詳細は[`phase4-design.md`](./phase4-design.md)の「2. プロジェクト単位のドキュメント管理」を参照
