...
## 4. チャットからの直接アクション実行(実装済み)

4項目の中で最もリスクが高い(検索して答えるだけだった`/chat`が、実際に副作用のある操作を行うようになる)ため、以下の制約を設けて実装した。

### スコープ

設計通り**「保存済みプロンプトでのAIレビュー実行」1種類のみ**に対応した。それ以外(リポジトリの接続解除、プロンプトの削除など破壊的な操作)はチャットからは一切実行できない。新しい実行用エンドポイントは追加せず、既存の`POST /api/repositories/:id/reviews`をそのまま呼び出す形にし、認可チェック・レート制限も既存のものをそのまま通している。

### 実現方式: Claudeのtool use + 確認ステップ

`src/lib/chat-action.ts`の`detectReviewActionIntent()`が、`POST /api/chat`の冒頭(埋め込み検索の前)でユーザーの発話をtool use(`propose_review_execution`ツール)で解析する。意図解析はユーザーがリポジトリ・プロンプトの両方を1件以上持っている場合のみ実行し(片方でも無ければ実行しようがない)、回答生成(`DEFAULT_MODEL`)とは別に低コストな`claude-haiku-4-5`を使う。

1. ユーザーの発話をtool useで解析し、実行したい操作とパラメータ(リポジトリID・PR番号・プロンプトID)の候補を抽出。ツールに渡す選択肢はそのユーザーが実際に持つリポジトリ・プロンプトの一覧のみとし、Claudeが実在しないIDを幻視した場合は提案自体を無効として通常のRAG回答にフォールバックする
2. ツール呼び出しがあった場合、`POST /api/chat`はRAG検索・回答生成を行わずに`{ actionProposal }`を返す。フロント(`src/app/(app)/chat/chat-panel.tsx`)はこれを受け取ると、既存の`ConfirmDialog`コンポーネントで「リポジトリ`owner/repo`のPR #123を、プロンプト『コードレビュー用』でレビューします」という確認内容を表示する
3. ユーザーが「実行」を押した場合のみ、フロントから`POST /api/repositories/:id/reviews`を呼び出す。「キャンセル」を押した場合は何も実行されない

人間の確認を挟むことで、AIが誤読したパラメータのまま実行される事故を防ぐ(このプロジェクト自体の開発でも、AIに任せきりにせず人間が最終確認する場面が重要だったのと同じ考え方)。

### トリガー元の記録

チャットから実行したレビューも、UIから実行したレビューと同じ`Review`テーブルに保存されるため既存の「レビュー履歴」タブ・レビュー詳細画面にそのまま表示されるが、どちらから実行されたかを見分けたいという要望を受け、`Review`に`triggeredVia`(`UI` | `CHAT`、デフォルト`UI`)を追加した。`POST /api/repositories/:id/reviews`のリクエストボディで`triggeredVia: "CHAT"`を指定した場合のみCHAT扱いとし、それ以外の値・未指定はすべてUI扱いにする(検証不要な安全側デフォルト)。レビュー履歴一覧・詳細画面には`CHAT`の場合のみ「チャットから実行」のバッジを表示する。
