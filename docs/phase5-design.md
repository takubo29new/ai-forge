...
対象: コードレビューに限定していたAI評価機能を、画像・テキストなど他の入力形式にも対象を広げる。アーキテクチャ・認証は [`phase1-design.md`](./phase1-design.md) を、DB設計の全体像は [`db-design.md`](./db-design.md) を参照。**画像評価(`inputType: IMAGE`)・テキスト評価(`inputType: TEXT`)・バックグラウンド処理を実装済み。画像の永続化(Vercel Blob等)・共有リンク・プロンプトテンプレート集は未着手。**
